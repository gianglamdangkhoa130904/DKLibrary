...
import com.example.finalvalue.Customer.PageActivity;
import com.example.finalvalue.Model.ChapterModel;
import com.example.finalvalue.R;

import java.util.ArrayList;

public class ListChapterAdapter extends RecyclerView.Adapter<ListChapterAdapter.MyViewHolder>{
    Context context;
    ArrayList<ChapterModel> arrChapter;

    public ListChapterAdapter(Context context, ArrayList<ChapterModel> arrChapter) {
        this.context = context;
        this.arrChapter = arrChapter;
    }
    @NonNull
    @Override
    public ListChapterAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_chapter, parent, false);
        return new MyViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ListChapterAdapter.MyViewHolder holder, int position) {
        ...
        holder.mainChapterLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                Intent a = new Intent(holder.itemView.getContext(), PageActivity.class);
                a.putExtra("chapter_id", arrChapter.get(position).idChapter);
                a.putExtra("chapter_des", arrChapter.get(position).chapDescription);
